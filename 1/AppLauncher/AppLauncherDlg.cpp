...
bool CAppLauncherDlg::CreatePopupOwned(int w, int h)
{
	...
	HWND hPopup = ::CreateWindowEx(
		...
		nullptr,
		dwStyle,
		200, 200, w, h,     // 初始屏幕坐标，位置逻辑完全不管理
		m_hWnd,             // Owner为主窗口
		NULL,               // Popup顶层窗口，hMenu必须NULL，解决1401
		AfxGetInstanceHandle(),
		nullptr);
	...
	if (!hPopup)
	{
		...
		TRACE1(_T("CreateWindowEx failed err=%u\n"), err);
		return false;
	}

	m_hPopupWnd = hPopup;
	TRACE1(_T("Popup created hPopup=%p\n"), m_hPopupWnd);
	return true;
}
#endif
void CAppLauncherDlg::OnActivate(UINT nState, CWnd* pWndOther, BOOL bMinimized)
{
	...
	// 主窗口得到焦点：把Popup提升到主窗口之上
	if (nState != WA_INACTIVE)
	{
		::ShowWindow(m_hPopupWnd, SW_SHOWNOACTIVATE);
		// Z序：popup放在 m_hWnd 的上面一级，不抢输入焦点
		::SetWindowPos(m_hPopupWnd, m_hWnd,
			0, 0, 0, 0,
			SWP_NOMOVE | SWP_NOSIZE | SWP_NOACTIVATE);
	}
}
...
void CAppLauncherDlg::SyncPopupCorner()
{
	...
	int x = rcMainScreen.right - m_popupW - 10;
	int y = rcMainScreen.top + 80;
